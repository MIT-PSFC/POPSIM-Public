...
def test_signal_rms_simple():
    ...
    signal_rms_params = SignalRMS.Params(signal=0)
    state_out, output = signal_rms(signal_rms_initial_state, signal_rms_params)
    assert output.rms == 0
    ...
    for i in range(window_size):
        ...
        signal_rms_params = SignalRMS.Params(signal=1)
        state_out, output = signal_rms(state_out, signal_rms_params)
...
def test_signal_rms_known():
    ...
    for i, _ in enumerate(x_vals):
        signal_rms_params = SignalRMS.Params(signal=signal[i])
        state_out, output = signal_rms(state_out, signal_rms_params)
