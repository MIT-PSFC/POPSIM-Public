...
def test_export(profile_predictor_latest_sparc):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(tmpdir / "model.json", "r") as f:
            ...
            n_layers = len(model_json['model_parameters']['profile_predictor']['nn']['layers'])
            for i in range(n_layers):
                wj = np.asarray(model_json['model_parameters']['profile_predictor']['nn']['layers'][i]['weight'])
                wm = trainer.train_state.model.profile_predictor.nn.layers[i].weight
                assert np.allclose(wj, wm), "First layer weights do not match!"
                
                bj = np.asarray(model_json['model_parameters']['profile_predictor']['nn']['layers'][i]['bias'])
                bm = trainer.train_state.model.profile_predictor.nn.layers[i].bias
