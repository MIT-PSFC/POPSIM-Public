...
from popsim.ml.dataloading import (
    DEFAULT_SAMPLE_DIM,
...
def test_time_dep_dl_nan_handling_no_nan(method):
    ...
    ds_expected = xr.Dataset(
        data_vars={
            ...
            "target_1D": (("sample", "time_idx_input", "rho"), np.array([[[4.00, 4.01, 4.02], [4.10, 4.11, 4.12], [4.20, 4.21, 4.22], [4.30, 4.31, 4.32]], [[4.00, 4.01, 4.02], [4.10, 4.11, 4.12], [4.20, 4.21, 4.22], [4.30, 4.31, 4.32]]])),
        },
        coords={
            "time": (("sample", "time_idx_input"), np.array([[0.0, 0.1, 0.2, 0.3], [0.0, 0.1, 0.2, 0.3]])),
            "input_batch": ("sample", [0, 0]),
            "shot": ("sample", [1, 2]),
            "rho": (("rho"), np.array([0, 0.5, 1])),
        },
    )
    ...
    [
        ...
        xr.Dataset(
            data_vars={
                ...
                "target_1D": (("sample", "time_idx_input", "rho"), np.array([[[4.00, 4.01, 4.02], [4.30, 4.31, 4.32], [4.30, 4.31, 4.32], [4.30, 4.31, 4.32]], [[4.00, 4.01, 4.02], [4.10, 4.11, 4.12], [4.20, 4.21, 4.22], [4.30, 4.31, 4.32]]])),
            },
            coords={
    ...
    [
        ...
        xr.Dataset(
            data_vars={
                ...
                "target_1D": (("sample", "time_idx_input", "rho"), np.array([[[4.00, 4.01, 4.02], [4.10, 4.11, 4.12], [4.20, 4.21, 4.22], [4.30, 4.31, 4.32]]])),
            },
            coords={
    ...
    [
        ...
        xr.Dataset(
            data_vars={
                ...
                "target_1D": (("sample", "time_idx_input", "rho"), np.array([[[4.00, 4.01, 4.02], [4.20, 4.21, 4.22], [4.30, 4.31, 4.32]], [[4.00, 4.01, 4.02], [4.10, 4.11, 4.12], [4.30, 4.31, 4.32]]])),
            },
            coords={
...
def test_time_dep_dl_dim_naming():
    """Non-windowed dims keep their true names and coords through the dataloader.
    The BatchGenerator suffixes every input dim with "_input" so batches concat positionally,
    but only the windowed time dim genuinely needs that. Check that:
    - the time dim keeps the "_input" suffix
    - a non-windowed dim with a coordinate (rho) keeps its name and gets its coord back as an indexed dim coordinate
    - a non-windowed dim without a coordinate (channel) keeps its name and stays coord-less
    """
    num_shots = 3
    num_times = 20
    segment_length = 11
    num_rho = 7
    num_channels = 4
    rhogrid = np.linspace(0, 1, num_rho)
    ds = xr.Dataset(
        {
            "ip": (("shot", "time_slice"), np.random.rand(num_shots, num_times)),
            "Wmhd": (("shot", "time_slice"), np.random.rand(num_shots, num_times)),
            "ne_rho": (("shot", "time_slice", "rho"), np.random.rand(num_shots, num_times, num_rho)),
            "bolo": (("shot", "time_slice", "channel"), np.random.rand(num_shots, num_times, num_channels)),
        },
        coords={
            "shot": np.arange(num_shots),
            "rho": rhogrid,
    ...
    # The windowed time dim keeps the "_input" suffix
    assert "time_slice_input" in dl.ds.dims
    assert dl.dataset.training_metadata.time_dep_metadata.time_dim == "time_slice_input"

    # Non-windowed dims keep their true names with no suffixed leftovers
    assert "rho" in dl.ds.dims
    assert "channel" in dl.ds.dims
    assert "rho_input" not in dl.ds.dims
    assert "channel_input" not in dl.ds.dims

    # The rho coordinate is restored as an indexed dimension coordinate with the original grid values
    assert dl.ds["rho"].dims == ("rho",)
    assert "rho" in dl.ds.indexes
    np.testing.assert_allclose(dl.ds["rho"].values, rhogrid)

    # The coord-less dim stays coord-less
    assert "channel" not in dl.ds.coords

    # The dim naming carries through to the batches
    for batch in dl:
