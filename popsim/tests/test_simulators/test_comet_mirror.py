...
def test_comet_mirror_prd():
    ...
    checks = {
        "P_fusion_MW": (final_aux["P_fusion_MW"], Sparc2020TestData.Pfusion, 15.0), # Relatively large error for some reason.
        "P_ohmic_MW": (final_aux["P_ohmic_MW"], Sparc2020TestData.Pohm, 20.0), # Relatively large error for some reason.
        "P_aux_MW": (final_aux['params']['P_aux_MW'], Sparc2020TestData.Paux, 1e-3), # Should be exact because it's a parameter.
        "beta_t": (final_aux["beta_t"], Sparc2020TestData.beta, 15.0),
        "tau_E": (final_aux["tau_E"], Sparc2020TestData.tauE, 10.0),
        "P_rad_MW": (final_aux["P_rad_MW"], Sparc2020TestData.Prad, 20.0),
        "ion_density": (final_state.density_state.total_volume_average_ion_density, Sparc2020TestData.ni_vol, 10.0),
        "average_electron_density_19": (final_aux["average_electron_density_19"], Sparc2020TestData.ne_vol, 5.0),
        "average_electron_temp_keV": (final_aux["average_electron_temp_keV"], Sparc2020TestData.Te_vol, 10.0), # Relatively large error for some reason.
        "average_ion_temp_keV": (final_aux["average_ion_temp_keV"], Sparc2020TestData.Ti_vol, 10.0), # Relatively large error for some reason.
        "q_star": (final_aux["q_star"], Sparc2020TestData.qstar_uckan, 10.0),
        "dilution": (final_aux["dilution"], Sparc2020TestData.dilution, 20.0), # Relatively large error for some reason.
        "z_effective": (final_aux["z_effective"], Sparc2020TestData.Zeff, 20.0), # Relatively large error for some reason.
        "PsepB0R0": (PsepB0R0, Sparc2020TestData.PsepB0R0, 15.0),
    }
