...
@pytest.mark.parametrize("arr, extra_dim_and_coords, expected_dims, expected_output, expect_warning", [
    (
    ...
    (
        # 3D array with simulation and time coordinates and one extra coord
        np.ones((2, 3, 4)),
        [("extra", np.arange(4))],
    ...
    (
        # 3D array with simulation and time coordinates and two extra coords, expect a user warning.
        np.ones((2, 3, 4)),
        [("extra", np.arange(4)), ("extra2", np.arange(4))],
        None,
        lambda out: out is None,
        True
    ),
])
def test_make_data_array(arr, extra_dim_and_coords, expected_dims, expected_output, expect_warning):
    nsim = 2
    ntime = 3
    base_coords = {"simulation": xr.DataArray(np.arange(nsim), dims=("simulation", )), "time": xr.DataArray(np.arange(ntime), dims=("time", ))}


    if expect_warning:
        with pytest.warns(UserWarning):
            out = make_data_array("test", arr, base_dims=["simulation", "time"], base_coords = base_coords, extra_dim_and_coord_spec=extra_dim_and_coords)
    else:
        out = make_data_array("test", arr, base_dims=["simulation", "time"], base_coords = base_coords, extra_dim_and_coord_spec=extra_dim_and_coords)
...
def test_make_data_array_value_error():
    nsim = 2
    ntime = 3
    base_coords = {"simulation": xr.DataArray(np.arange(nsim), dims=("simulation", )), "time": xr.DataArray(np.arange(ntime), dims=("time", ))}
    arr = np.ones((2, 3, 4))
    extra_dim_and_coord_spec = [("extra", np.arange(5))]

    with pytest.raises(ValueError):
        out = make_data_array("test", arr, base_dims=["simulation", "time"], base_coords = base_coords, extra_dim_and_coord_spec=extra_dim_and_coord_spec)
...
def test_extra_dimensions_and_coordinates():
    # Case 1: full specification of the extra dimensions and coordinates
    time = np.arange(10)
    tree = {
        'a': np.random.rand(10, 5),
        'b': {
            'c': np.random.rand(10, 3, 2),
            'd': np.random.rand(10, 4)
        }
    }

    extra_dim_and_coord_tree = {
        'a': ("x", np.arange(5)),
        'b': {
            'c': [("y", np.arange(3)), ("z", np.arange(2))],
            'd': ("w", np.arange(4))
        }
    }
    
    result = time_and_pytree_to_xarray(time, tree, extra_dim_and_coord_tree)
    
    assert isinstance(result, xr.Dataset)
    assert set(result.dims) == {'time', 'x', 'y', 'z', 'w'}
    assert result.sizes['time'] == 10
    assert result.sizes['x'] == 5
    assert result.sizes['y'] == 3
    assert result.sizes['z'] == 2
    assert result.sizes['w'] == 4
    assert set(result.data_vars) == {'a', 'b.c', 'b.d'}

    # Case 2: incomplete specification of b.c. Expect a warning.
    extra_dim_and_coord_tree = {
        'a': ("x", np.arange(5)),
        'b': {
            'c': [("y", np.arange(3))],
    ...
    # Case 3: don't specify b.c., expect it to automatically create the extra dimensions and coordinates.
    extra_dim_and_coord_tree = {
        'a': ("x", np.arange(5)),
        'b': {
            'c': None,
