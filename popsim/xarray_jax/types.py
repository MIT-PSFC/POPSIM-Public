...
class Variable(eqx.Module):
    ...
    def __init__(self, data: jax.Array, dims: tuple[str, ...], attrs: Optional[Mapping] = None):
        self.data = data
        self.dims = dims
        self.attrs = attrs
    ...
    @classmethod
    def from_xarray(cls, var: xarray.Variable) -> "Variable":
...
class DataArray(eqx.Module):
    ...
    def __init__(self, variable: Variable, coords: Mapping[Hashable, xarray.Variable], name: Optional[str] = None):
    ...
    @classmethod
    def from_xarray(cls, da: xarray.DataArray) -> "DataArray":
        return cls(Variable.from_xarray(da.variable), _HashableCoords(da.coords), da.name)
...
class Dataset(eqx.Module):
    ...
    def __init__(self, variables: dict[Hashable, Variable], coords: Mapping[Hashable, xarray.Variable], attrs: Optional[Mapping] = None):
    ...
    def from_xarray(cls, ds: xarray.Dataset) -> "Dataset":
        return cls(
            {name: Variable.from_xarray(da.variable) for name, da in ds.data_vars.items()},
