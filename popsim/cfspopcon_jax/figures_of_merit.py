...
def calc_rho_star(
    ...
    Returns:
         rho_star [~]
    """
    return np.sqrt(fuel_average_mass_number * average_ion_temp) / (_UREGE * magnetic_field_on_axis * minor_radius)
...
def calc_coulomb_logarithm(ne: float, Te: float) -> float:
    ...
    """
    return 30.9 - np.log(ne**0.5 * Te**-1.0)
...
def calc_normalised_collisionality(
    ...
    return (
        _UREGE**4
        / (2.0 * np.pi * 3**1.5 * _UREGEPSILON0**2)
...
def calc_peak_pressure(
    ...
    Returns:
         peak_pressure [Pa]
    """
    return peak_electron_temp * peak_electron_density + peak_ion_temp * peak_fuel_ion_density
