...
def calc_unit_conversion_factor():
    ...
    non_dim_unit = ((2 * mu_0 * n * (T + T)) / (B**2)).to("")
    return non_dim_unit.magnitude


def pressure(average_electron_density, average_electron_temp, average_ion_temp):
    ev_to_J = 1.602176634e-19
    res = 1e19 * average_electron_density * (1000 * ev_to_J * average_electron_temp + 1000 * ev_to_J * average_ion_temp)
    return res
...
def _calc_beta_general(
    ...
    ret = 2 * const.mu_0 * pressure(average_electron_density, average_electron_temp, average_ion_temp) / (magnetic_field**2)
    return ret
...
def calc_beta_poloidal(
    ...
    units_conversion_factor = const.mu_0 / (2 * np.pi)
    B_pol = units_conversion_factor * 1e6 * plasma_current / minor_radius
